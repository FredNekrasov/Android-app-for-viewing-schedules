...
class GroupRepository(private val api: IGroupService) : IRepository<SquadDto> {
    override suspend fun getDTOList(): StateFlow<Pair<ConnectionType,List<SquadDto>>> {
        val data = MutableStateFlow(Pair(LOADING,emptyList<SquadDto>()))
        try {
