...
class UserRepository(private val api: IUserService) : IRepository<UserDto> {
    override suspend fun getDTOList(): StateFlow<Pair<ConnectionType,List<UserDto>>> {
        val data = MutableStateFlow(Pair(LOADING,emptyList<UserDto>()))
        try {
