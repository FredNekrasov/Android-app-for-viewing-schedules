...
class SemesterRepository(private val api: ISemesterService) : IRepository<SemesterDto> {
    override suspend fun getDTOList(): StateFlow<Pair<ConnectionType,List<SemesterDto>>> {
        val data = MutableStateFlow(Pair(LOADING,emptyList<SemesterDto>()))
        try {
