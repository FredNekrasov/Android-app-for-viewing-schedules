package com.schedule.ui.presentation.data_list.teacher.view_model
...
@HiltViewModel
class TeacherVM @Inject constructor(private val useCase: IUseCase<Teacher>) : ViewModel() {
    private var dataMSF = MutableStateFlow(LOADING to emptyList<Teacher>())
            ...
            useCase.getList().collectLatest {
                val second = async {
                    it.second.filter { entity ->
                        entity.name.startsWith(txt) || entity.surname.startsWith(txt) || entity.patronymic.startsWith(txt)
                    }
                }
                dataMSF.emit(Pair(it.first, second.await()))
            }
        }
    }
}